...
| Dynamic + causal Teacher | Ding 2026 TASE | bearing dynamics prior | causal prior抑制 confounding | teacher representation | interpretable KD | ★★★★☆ |
...
### C — Mechanism-only
仅允许 Fault + Geometry + necessary RPM/Load。目标不是拟合完整真实波形，而是生成可靠 fault evidence。

### B — Full-physics-like
在 C 基础上加入 housing/support/transfer path/sensor response，研究更多 machine-specific information 是否提高或损害 transferability。

### A — Signal-faithful
进一步用真实数据校准参数，使 PSD/envelope/statistical distributions 更接近实测。这里要警惕“越像真实信号，越可能把 nuisance cue 一起学进去”。
