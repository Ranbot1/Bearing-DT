...
- Journal: **Knowledge-Based Systems**, 292, 111641
...
不同 fault classes 样本严重不平衡。单纯使用手设参数的 bearing simulator 产生的数据和真实信号差距大。
...
- 我们拟研究：**是否所有提高的一致性都值得 transfer 给 classifier**。

非常适合做 A-vs-C 的理论对照。

## 复现风险

inverse problem 有可辨识性问题：多个参数组合可能产生相似 spectrum。论文的诊断性能提高不自动证明 (hat{	heta}) 就是真实物理参数。
